...
class DecisionTree:
    ...
    def get_lines(self, X): 
        X_data= X.copy() # Make a copy of original data X. You should sort or modify X_data below in this method instead of changing original data X. 
        lines = [] 
        # Write your code here:
        for direction in [0,1]:  
            # There are N+1 midpoints for N values in range [0,1]
            curr_X = list(X_data[:,direction])
            curr_X.insert(0,0)
            curr_X.append(1)
            curr_X.sort()
            
            for i in range(X_data.shape[0]+1):
            # Find the midpoint of each interval as the position. 
                position = (curr_X[i]+curr_X[i+1])/2
                line = Line(direction, position)
                lines.append(line)
    ...
    def calculate_line(self, line, X, Y): 
        impurity_value = 0
        impurity_value_right = 0
        impurity_value_left = 0
        # Write your code here:
        mask1 = X[:,line.direction] < line.position
        mask2 = X[:,line.direction] >= line.position
        y_right =  Y[mask1]
        y_left = Y[mask2]
        p1_right = 1 if len(y_right) == 0 else abs(sum(y_right[y_right==-1]))/len(y_right)
        p2_right = 1 if len(y_right) == 0 else sum(y_right[y_right==1])/len(y_right)
        p1_left = 1 if len(y_left) == 0 else abs(sum(y_left[y_left==-1]))/len(y_left)
        p2_left =  1 if len(y_left) == 0 else sum(y_left[y_left== 1])/len(y_left)
        impurity_value_right = p1_right*(1-p1_right)+p2_right*(1-p2_right)
        impurity_value_left = p1_left*(1-p1_left)+p2_left*(1-p2_left)
         
        impurity_value = len(y_right)*impurity_value_right+len(y_left)*impurity_value_left
    ...
    def split(self, node):
        ...
        for child in [node.left, node.right]:
            if child.get_number_of_points() > self.min_samples_split:
                if child.depth < self.max_depth:
                    if child.get_impurity() != 0:
                        print
                        self.split(child)
